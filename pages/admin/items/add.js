...
function add() {
    ...
    const [providerName, setProviderName] = useState(""); //TODO: Add provider selection
    const [providerDocRef, setProviderDocRef] = useState("");
    ...
    const searchForProvider = async () => {
        const scheduleRef = collection(db, 'providers');
        const q = query(scheduleRef, where("name", '==', providerRef.current.value ));
        

        const querySnapshot = await getDocs(q);

        if(querySnapshot.empty) {
            setProviderInvalid(true);
            setProviderValidated(false);
        }

        querySnapshot.forEach((doc) => {
            setProviderName(doc.data().name);

            setProviderValidated(true);
            setProviderDocRef(doc.id);
            setProviderInvalid(false);
        });
    }
    ...
    const createItem = async () => {
        ...
        if(!nameRef.current.value || !descRef.current.value || !expectedDeliveryTimeRef.current.value ||
        ...
        const docRef = await addDoc(collection(db, 'items'), {
            ...
            expectedDeliveryTime: Number(expectedDeliveryTimeRef.current.value)
        }).then(async (snapshot) => {
                            ...
                            <div className='mt-5 '>
                                ...
                                <div className='items-end align-sub mt-3'>
                                    ...
                                    <div className='mt-2'>
                                        ...
                                        placeholder="Expected Delivery Time Minutes" />
                                    </div>
                                </div>
                            </div>
                        </div>
